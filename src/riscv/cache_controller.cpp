...
bool ccSimulate(CacheController* cc, const MemoryRequest* cpuReq, MemoryResponse* cpuRes, MemoryRequest* memReq, const MemoryResponse* memRes)
{
	...
	cc->m_HitBlockID = cacheLookup(cc->m_Cache, cc->m_State.m_LookupAddr + cc->m_State.m_WordByteOffset, cc->m_Tag, cc->m_SetID, cc->m_BlockOffset);
...
void cc_Lookup(CacheController* cc, const MemoryRequest* cpuReq, MemoryResponse* cpuRes, MemoryRequest* memReq, const MemoryResponse* memRes)
{
	...
	if (cc->m_HitBlockID != ~0u) {
		// Cache hit.
		const uint32_t tagBits = cacheGetBlockTagBits(cc->m_Cache, cc->m_SetID, cc->m_HitBlockID);
		...
		cacheSetBlockTagBits(cc->m_Cache, cc->m_SetID, cc->m_HitBlockID, newTagBits);
...
void cc_HitRead(CacheController* cc, const MemoryRequest* cpuReq, MemoryResponse* cpuRes, MemoryRequest* memReq, const MemoryResponse* memRes)
{
	...
	uint32_t numBytesRead = cacheRead(cc->m_Cache, cc->m_SetID, cc->m_HitBlockID, cc->m_BlockOffset, cc->m_State.m_ReqSize, ptr);
...
void cc_HitWrite(CacheController* cc, const MemoryRequest* cpuReq, MemoryResponse* cpuRes, MemoryRequest* memReq, const MemoryResponse* memRes)
{
	...
	uint32_t numBytesWritten = cacheWrite(cc->m_Cache, cc->m_SetID, cc->m_HitBlockID, cc->m_BlockOffset, cc->m_State.m_ReqSize, ptr);
...
void cc_Miss(CacheController* cc, const MemoryRequest* cpuReq, MemoryResponse* cpuRes, MemoryRequest* memReq, const MemoryResponse* memRes)
{
	...
	const uint32_t emptyBlockID = cacheGetEmptyBlock(cc->m_Cache, cc->m_SetID);
	const uint32_t randomBlockID = cc->m_State.m_TickCounter % cacheGetNumWays(cc->m_Cache);
	const uint32_t victimBlockID = emptyBlockID != ~0u ? emptyBlockID : randomBlockID;

	const uint32_t victimBlockTagBits = cacheGetBlockTagBits(cc->m_Cache, cc->m_SetID, victimBlockID);
	...
	if (cacheRead(cc->m_Cache, cc->m_SetID, victimBlockID, 0, 4, &victimBlockWord) != 4) {
		RISCV_CHECK(false, "Failed to read word from cache block");
	}

	const uint32_t wordAddr = cacheMakeAddress(cc->m_Cache, needsWriteBack 
	...
	cacheSetBlockTagBits(cc->m_Cache, cc->m_SetID, victimBlockID, cacheBlockMakeTagBits(needsWriteBack ? cacheBlockGetTag(victimBlockTagBits) : cc->m_Tag, !needsWriteBack, false, false));
...
void cc_Allocate(CacheController* cc, const MemoryRequest* cpuReq, MemoryResponse* cpuRes, MemoryRequest* memReq, const MemoryResponse* memRes)
{
	...
	const uint32_t activeBlockTagBits = cacheGetBlockTagBits(cc->m_Cache, activeSetID, activeBlockID);
	...
	if (!memRes->m_Control.m_Fields.m_Valid) {
		...
		cacheSetBlockTagBits(cc->m_Cache, activeSetID, activeBlockID, cacheBlockMakeTagBits(activeBlockTag, true, false, true));
		cc->m_NextState.m_ID = CacheController::State::Lookup;
	} else {
		if (cacheWrite(cc->m_Cache, activeSetID, activeBlockID, cc->m_State.m_ActiveBlockOffset, 4, &memRes->m_Data) != 4) {
			RISCV_CHECK(false, "Failed to write word to cache block");
		}

		const uint32_t newBlockOffset = cc->m_State.m_ActiveBlockOffset + 4;
		const bool hasMoreWords = newBlockOffset != cacheGetBlockSize(cc->m_Cache);
		const uint32_t wordAddr = cacheMakeAddress(cc->m_Cache, activeBlockTag, activeSetID, newBlockOffset);
...
void cc_WriteBack(CacheController* cc, const MemoryRequest* cpuReq, MemoryResponse* cpuRes, MemoryRequest* memReq, const MemoryResponse* memRes)
{
	...
	if (!memRes->m_Control.m_Fields.m_Valid) {
		...
		uint32_t val;
		if (cacheRead(cc->m_Cache, activeSetID, activeBlockID, newBlockOffset % cacheGetBlockSize(cc->m_Cache), 4, &val) != 4) {
			RISCV_CHECK(false, "Failed to read word from cache block");
		}
...
void cc_Flush(CacheController* cc, const MemoryRequest* cpuReq, MemoryResponse* cpuRes, MemoryRequest* memReq, const MemoryResponse* memRes)
{
	...
	cacheSetBlockTagBits(cc->m_Cache, activeSetID, activeBlockID, cacheBlockMakeTagBits(activeBlockTag, false, false, false));
	...
	if (cacheBlockIsValid(activeBlockTagBits) && cacheBlockIsDirty(activeBlockTagBits) && !cacheBlockIsError(activeBlockTagBits)) {
		...
		uint32_t val;
		if (cacheRead(cc->m_Cache, activeSetID, activeBlockID, 0, 4, &val) != 4) {
			RISCV_CHECK(false, "Failed to read word from cache block");
		}

		const uint32_t wordAddr = cacheMakeAddress(cc->m_Cache, activeBlockTag, activeSetID, 0);
		...
		if (cc->m_NextState.m_ActiveBlockID == cacheGetNumWays(cc->m_Cache)) {
			cc->m_NextState.m_ActiveBlockID = 0;
			cc->m_NextState.m_ActiveSetID = activeSetID + 1;

			if (cc->m_NextState.m_ActiveSetID == cacheGetNumSets(cc->m_Cache)) {
				cc->m_NextState.m_ActiveSetID = 0;
				done = true;
			}
		}
